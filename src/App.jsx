...
const wxEmoji = (f = "") => {
  ...
  if (s.includes("heavy rain")) return "🌧";
  if (s.includes("rain") || s.includes("shower")) return "🌦";
  if (s.includes("cloudy"))     return "☁️";
  if (s.includes("hazy"))       return "🌫";
  if (s.includes("fair"))       return "☀️";
  if (s.includes("windy"))      return "💨";
  return "🌤";
};
const wxBg = (f = "") => {
  const s = f.toLowerCase();
  if (s.includes("thunder") || s.includes("heavy rain")) return "#2d3748";
  if (s.includes("rain") || s.includes("shower"))        return "#2b6cb0";
  if (s.includes("fair"))                                 return "#276749";
  return "#4a5568";
};
...
function useWeather() {
  ...
  const fetch2h = useCallback(async () => {
    try {
      ...
      const central = forecasts.find(f => ["Jurong West","Jurong East","Boon Lay","Clementi","Bukit Batok"].includes(f.area)) || forecasts[0];
      setWx({ area: central.area, forecast: central.forecast });
      setError(false);
    } catch { setError(true); }
  }, []);
  ...
  const fetch24h = useCallback(async () => {
    try {
      ...
      const items = d?.data?.items;
      if (!items?.length) throw new Error();
      setWx24({ general: items[0].general, periods: items[0].periods });
    } catch {}
  }, []);
...
export default function App() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [u, s, p] = await Promise.all([
        db("users?select=*&order=created_at.asc"),
        db("slots?select=*&order=created_at.desc"),
        db("polls?select=*&order=created_at.desc"),
      ]);
      setUsers(u); setSlots(s); setPolls(p);
    } catch (e) { console.error(e); setDbErr(true); }
  }, []);
    ...
    if (authMode === "register") {
      ...
      try {
        const [user] = await db("users", {
          method: "POST",
          body: JSON.stringify({ id: uid(), name, pin }),
        });
        localStorage.setItem("hb_user", JSON.stringify(user));
        setCurrentUser(user);
        await loadAll();
        showToast(`Welcome, ${name}! 🎾`);
      } catch { setAuthErr("Couldn't create account. Try again."); return; }
    } else {
  ...
  const postSlot = async () => {
    ...
    try {
      await db("slots", {
        method: "POST",
        body: JSON.stringify({
          id: uid(), date, time_from: timeFrom, time_to: timeTo,
          court, notes: newSlot.notes,
          posted_by: currentUser.id, posted_by_name: currentUser.name,
          signups: [], created_at: Date.now()
        }),
      });
    ...
    } catch { showToast("Couldn't post slot.", "err"); }
  };

  const toggleSignup = async (slot) => {
    const already = slot.signups.includes(currentUser.id);
    const updated = already
      ? slot.signups.filter(id => id !== currentUser.id)
      : [...slot.signups, currentUser.id];
    try {
      await db(`slots?id=eq.${slot.id}`, {
        method: "PATCH",
        body: JSON.stringify({ signups: updated }),
        prefer: "return=minimal",
      });
      await loadAll();
    } catch { showToast("Couldn't update.", "err"); }
  };

  const deleteSlot = async (id) => {
    try {
      await db(`slots?id=eq.${id}`, { method: "DELETE", prefer: "return=minimal" });
      await loadAll();
      showToast("Slot removed.");
    } catch { showToast("Couldn't delete.", "err"); }
  };
  ...
  const postPoll = async () => {
    ...
    } catch { showToast("Couldn't create poll.", "err"); }
  };
  ...
  const votePoll = async (poll, optIdx) => {
    const updatedOptions = poll.options.map((o, i) => {
      ...
      return { ...o, votes: voted ? o.votes.filter(id => id !== currentUser.id) : [...o.votes, currentUser.id] };
    });
    try {
      await db(`polls?id=eq.${poll.id}`, {
        method: "PATCH",
        body: JSON.stringify({ options: updatedOptions }),
        prefer: "return=minimal",
      });
      await loadAll();
    } catch { showToast("Couldn't save vote.", "err"); }
  };

  const deletePoll = async (id) => {
    try {
      await db(`polls?id=eq.${id}`, { method: "DELETE", prefer: "return=minimal" });
      await loadAll();
      ...
      <main style={S.main}>
        ...
        {!currentUser && (
          <div style={S.authCard}>
            ...
            {dbErr && <p style={S.errMsg}>⚠️ Database connection issue. Try refreshing.</p>}
        ...
        {currentUser && view==="venues" && (
          <div>
            ...
            <div style={S.filterRow}>
              {["All","SAFRA","ActiveSG","Kallang","East","Tanglin","Holland"].map(area => (
            ...
            <div style={S.venueNote}>ℹ️ These links go directly to each venue's booking portal.</div>
          </div>
        )}
...
function WeatherSnapshot({ wx, wx24, loading, error, onMore }) {
  ...
  return (
    <div style={{...S.wxSnap, background: bg}} onClick={onMore}>
      {loading ? <div style={S.wxSnapText}>☁️ Loading weather…</div>
      : error || !wx ? (
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <a href="https://www.nea.gov.sg/weather" target="_blank" rel="noopener noreferrer"
            style={{color:"#b7e4c7",fontSize:12,fontWeight:600,textDecoration:"none"}}>NEA site →</a>
        </div>
      ) : (
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
          ...
          {wx24?.general && (
            <div style={S.wxSnapRight}>
              ...
              <div style={S.wxSnapRightTemp}>{wx24.general.temperature?.low}°–{wx24.general.temperature?.high}°C</div>
              <div style={S.wxSnapRightHum}>💧 {wx24.general.relativeHumidity?.low}–{wx24.general.relativeHumidity?.high}%</div>
...
function WeatherFull({ wx, wx24, loading, error }) {
  ...
  if (error || !wx) return (
    <div style={{...S.wxBig, background:"#4a5568", textAlign:"center"}}>
      ...
      <a href="https://www.nea.gov.sg/weather" target="_blank" rel="noopener noreferrer"
        style={{color:"#b7e4c7",fontSize:13,fontWeight:600,textDecoration:"none"}}>View NEA weather directly →</a>
    </div>
  );
  const forecast = wx?.forecast || "";
  const isGood = !forecast.toLowerCase().includes("rain") && !forecast.toLowerCase().includes("thunder");
  return (
    <div>
      <div style={{...S.wxBig, background: wxBg(forecast)}}>
        <div style={S.wxBigEmoji}>{wxEmoji(forecast)}</div>
        ...
        <div style={{...S.wxBigAdvice, color: isGood?"#b7e4c7":"#fbd38d"}}>
          {isGood ? "✅ Conditions look good — get out there!" : "⚠️ Might want to wait this one out"}
        </div>
      </div>
      {wx24?.general && (
        <div style={S.wxCard}>
            ...
            return (
              <div key={i} style={S.periodRow}>
                ...
                <div style={S.periodRegions}>
                  {Object.entries(period.regions||{}).map(([region,fc]) => (
      ...
      <div style={S.wxSource}>Data source: NEA Singapore · Refreshed every 10 min</div>
    </div>
  );
}
...
function SlotCard({ slot, currentUser, users, onSignup, onDelete }) {
  ...
  const isOwner     = slot.posted_by === currentUser.id;
    ...
    <div style={S.card}>
      <div style={S.cardTop}>
        <div>
          ...
          <div style={S.cardTime}>{fmtTime(slot.time_from)} – {fmtTime(slot.time_to)}</div>
          <div style={S.cardCourt}>📍 {slot.court}</div>
          {slot.notes && <div style={S.cardNotes}>{slot.notes}</div>}
        </div>
        <div style={{display:"flex",gap:8,alignItems:"flex-start",flexShrink:0}}>
          <button style={{...S.btnSignup,...(isSignedUp?S.btnSignupActive:{})}} onClick={() => onSignup(slot)}>
      ...
      <div style={S.cardFooter}>
        <span style={S.postedBy}>Posted by {slot.posted_by_name}</span>
...
function PollCard({ poll, currentUser, users, onVote, onDelete }) {
  const isOwner     = poll.created_by === currentUser.id;
  ...
  return (
    <div style={S.card}>
      ...
      {(poll.options||[]).map((opt,i) => {
        ...
        return (
          <div key={i} style={S.pollOption} onClick={() => onVote(poll, i)}>
      ...
      <div style={{...S.cardFooter,marginTop:12}}>
        <span style={S.postedBy}>By {poll.created_by_name}</span>
...
const S = {
  ...
  logoBadge:{ fontSize:10, fontWeight:700, color:"#b7e4c7", background:"rgba(255,255,255,0.15)", padding:"2px 6px", borderRadius:4 },
  ...
  venueGrid: { display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))", gap:12, marginBottom:16 },
  venueCard: { background:"#fff", borderRadius:12, padding:"16px 14px", border:"1px solid #e2e8f0", textDecoration:"none", color:"#2d3748", display:"block" },
